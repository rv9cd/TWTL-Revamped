...
def mark_product(dfa_src1, dfa_src2, dfa_dest, operation, choices=None):
    ...
    '''
    final_dest = next(iter(dfa_dest.final))
    final_src1 = next(iter(dfa_src1.final))
    final_src2 = next(iter(dfa_src2.final))
...
def hold(props, prop, duration, negation=False):
    ...
    ngen = it.count()
    nodes = [next(ngen) for _ in range(duration + 2)]
...
def union(dfa1, dfa2):
    ...
    if len(dfa.final) > 1:
        ...
        dfa.g.remove_nodes_from(dfa.final - {final})
        dfa.final = {final}
...
def eventually(phi_dfa, low, high):
    ...
    if low > 0:
        ...
        nodes = [next(ngen) for _ in range(low)]
        attr_dict = {'weight': 0, 'input': bitmaps, 'guard': guard, 'label': guard}
        dfa.g.add_path(nodes, **attr_dict)
...
def repeat(phi_dfa, low, high):
    ...
    for rstate in inits[1:]:  # current restart state
        ...
        current_states = next_states | {rstate}
