...
class Ts(Model):
    ...
    def read_from_file(self, path):
        ...
        try:
            ...
            raise FileError(
                "Line 1 must be of the form: 'name name_of_the_transition_system', read: '%s'." % lines[line_cnt])
        ...
        try:
            ...
            raise FileError(
                "Line 2 must give the initial distribution of the form {'state_label': 1}, read: '%s'." % lines[
                    line_cnt])
        ...
        for init in self.init:
            if self.init[init] != 1:
                raise FileError(
                    'Initial probability of state %s cannot be %f in a transition system.' % (init, self.init[init]))
