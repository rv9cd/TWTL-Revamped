...
from lomap.markov import Markov
	...
	while(stack):
		...
		for tran_tuple in it.product(*map(lambda t,q: t.next_states_of_wts(q), ts_tuple, cur_state)):
			...
			if(next_state not in product_ts.g):
				...
				# Add transition w/ weight
				product_ts.g.add_edge(cur_state, next_state, attr_dict = {'duration':duration})
...
def markov_times_markov(markov_tuple):
	...
	while(stack):
		...
		# Actual source states of traveling states
		source_state = tuple(map(lambda q: q[0] if isinstance(q, tuple) and len(q)==3 and isinstance(q[2], (int, float)) else q, cur_state))
		# Time spent since actual source states
		time_spent = tuple(map(lambda q: q[2] if isinstance(q, tuple) and len(q)==3 and isinstance(q[2], (int, float)) else 0, cur_state))
