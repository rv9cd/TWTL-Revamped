...
from lomap.product import ts_times_ts
...
def expand_duration_ts(ts):
    ...
    for u, v, data in ts.g.edges_iter(data=True):
        ...
        edge_list = zip(list(zip(aux_nodes[:-1], aux_nodes[1:])) + list(zip(aux_nodes, aux_nodes)))
        # If you need a zip object again, you can create one
...
def verify(ts, dfa):
    ...
    holds.
    '''
    # assert dfa.kind == DFAType.Normal
