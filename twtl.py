...
def quant_translate(q_formula):
    f = q_formula.split(".")
    # print f
    quant = list(f[0])
    quant = [i for i in quant if i == 'F' or i == 'E']
    form = f[1]
    # print form
    # form = form.replace('_v1', '').replace('_v2', '')
    x = 0
    form_dict = dict()
    new = {}
    ops = ['*', '&', '->', '|', '!', '=']
    ...
    while i < len(quant):
        ...
        input_form = form_dict[i].split()

        for a in input_form:
            if '_' in a and p not in a:
                input_form.remove(a)

        for a in input_form:
            if p in a:
                res = ' '
                idx = input_form.index(a)
                res = input_form[idx].replace(p, q)
                input_form[idx] = res
                # print input_form
        # print input_form

        txt = [a for a in range(len(input_form))]
        for a in txt:
            new[a] = input_form[a]
        ...
        for key in new:
            if 'H' in new[key]:
                if new[key + 1] in ops:
                    new[key + 1] = ''
                if new[key + 2] in ops and 'R' not in new[key + 3] and 'H' not in new[key + 3]:
                    new[key + 2] = ''
                if ']^[' in new[key + 1]:
                    # print new[key]
                    new[key - 1] = ''
        ...
        for key in new:
            if 'H' in new[key]:
                form_str = form_str + str(new[key]) + ' '
            else:
                form_str = form_str + str(new[key])
        final_form.append(form_str)
        # print final_form
        if i > len(quant):
            break
        i += 1
    ...
    x = 0
    # print 'form', final_form
    formula = ''
    for x in range(len(final_form)):
        # print x
        if 'E' in quant:
            if x == (len(final_form) - 1):
                formula = formula + final_form[x]
            else:
                formula = formula + final_form[x] + ' ' + '|' + ' '
        else:
            if x == (len(final_form) - 1):
